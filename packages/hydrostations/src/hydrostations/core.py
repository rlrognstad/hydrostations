...
from hydrostations.register import build_adapters
from hydrostations.schema import COMPARTMENTS, empty_stations_frame, validate_stations_frame


def _default_registry() -> dict[str, SourceAdapter]:
...
def get_stations(
    *,
    ...
    """Fetch stations across one or more sources as a normalized GeoDataFrame.

    `basin` is resolved to a bounding box for spatial filtering -- a
    temporary approach pending the HydroBASINS crosswalk (see
    `hydrostations.basins`). Sources with a not-yet-implemented adapter are
    silently skipped when `source` is unset (a broad query), but raise if
