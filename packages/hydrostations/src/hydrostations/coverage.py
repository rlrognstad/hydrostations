"""Static network/compartment coverage lookup by geographic area.

Answers "which networks and compartments are declared to cover this area?"
...
+ AK/HI/PR; WISE = a Europe bounding box) meant for a quick "is this network
even worth trying here" check -- not authoritative service-area boundaries.
See each adapter module for its specific coverage bboxes.
"""
...
def lookup_coverage(polygon: BaseGeometry) -> dict[str, list[str]]:
    """Which networks (and their compartments) declare coverage overlapping `polygon`.

    Returns `{network: [compartment, ...]}` for every adapter whose declared
    ...
    for adapter in _default_registry().values():
        ...
        if any(b.intersects(polygon) for b in bboxes):
            result[adapter.network] = list(adapter.compartments)
    return result
